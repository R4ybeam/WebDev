...
try {
    ...
    echo 'Databasefout bij het ophalen van berichten: ' . $e->getMessage();
    exit;
}
...
if (sizeof($items) > 0) {
    ...
    <ul>
        ...
        foreach ($items as $item) {
            ?>
            <li><?php echo htmlspecialchars($item['sender']); ?> :
                <?php echo htmlspecialchars($item['email']); ?> :
                <?php echo htmlspecialchars($item['message']); ?> (
                <?php echo (new Datetime($item['added_on']))->format('d-m-Y H:i:s'); ?>)
            </li>
            <?php
        }
        ?>
